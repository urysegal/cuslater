...
namespace cuslater {
    __global__ void evalIntegrand_3DBloackReduce(int n, real_t hx, real_t hy, real_t hz, real_t az,
                                                 real_t r, real_t lx, real_t ly, real_t lz,
        ...
        real_t zoffc3 = c8 - r * lz;
        real_t zoffc4 = c11 - r * lz;
        ...
        real_t local = 0.f;
        if (__builtin_expect(x < 200 && y < 200, 1)) [[likely]] {
...

            {
                ...
                real_t zdiffc_3 = zvalue - zoffc3;
                real_t zdiffc_4 = zvalue - zoffc4;
    ...
    double evaluateFourCenterIntegral(real_t* c, real_t* alpha, int nr, int nl, int n,
        ...
        constexpr dim3 threads(16, 16, 4);
        const dim3     blocks((n + threads.x - 1) / threads.x, (n + threads.y - 1) / threads.y,
                              (n + threads.z - 1) / threads.z);
        std::cout << "Total Blocks: " << blocks.x * blocks.y * blocks.z << std::endl;
        std::cout << "Total Threads: "
                  << blocks.x * blocks.y * blocks.z * threads.x * threads.y * threads.z << std::endl;
        ...
        static thrust::device_vector<real_t> d_block_sums(blocks.x * blocks.y * blocks.z);
        static thrust::host_vector<real_t>   block_sums(blocks.x * blocks.y * blocks.z);
        ...
        for (int j = 0; j < nl; ++j) {
            for (int i = 0; i < nr; ++i) {
                evalIntegrand_3DBloackReduce<<<blocks, threads>>>(
                    n, hx, hy, hz, az, r_nodes[i], l_nodes_x[j], l_nodes_y[j], l_nodes_z[j],
