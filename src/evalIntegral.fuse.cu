...
__constant__ real_t d_lx[600];
__constant__ real_t d_ly[600];
__constant__ real_t d_lz[600];
__constant__ real_t d_lw[600];

namespace cuslater {

    // blocks = (n * n, nr, nl), threads = THREADS_PER_BLOCK
    // blocks = (, nr, nl), threads = THREADS_PER_BLOCK
    __global__ void evalIntegral_All(int n, int nr, real_t hx, real_t hy, real_t hz,
                                     double* __restrict__ out_rl) {
        // blocks = (n * n, nr, nl), threads = THREADS_PER_BLOCK
        ...
        real_t r  = d_r_nodes[ir];
        real_t rw = d_r_weights[ir];
        ...
        if (threadIdx.x == 0) atomicAdd(&out_rl[il * nr + ir], block_sum * (rw * lw));
    }
    ...
    double evaluateFourCenterIntegral(real_t* c, real_t* alpha, int nr, int nl, int nx, int ny, int nz,
        ...
        cudaMemcpyToSymbol(d_lx, l_nodes_x.data(), nl * sizeof(real_t));
        cudaMemcpyToSymbol(d_ly, l_nodes_y.data(), nl * sizeof(real_t));
        cudaMemcpyToSymbol(d_lz, l_nodes_z.data(), nl * sizeof(real_t));
        cudaMemcpyToSymbol(d_lw, l_weights.data(), nl * sizeof(real_t));

        cudaMemcpyToSymbol(d_r_nodes, r_nodes.data(), nr * sizeof(real_t));
        cudaMemcpyToSymbol(d_r_weights, r_weights.data(), nr * sizeof(real_t));
        ...
        thrust::device_vector<double> d_out(nr * nl, 0.0);

        double sum = 0.0;
        int    n   = nx; // number of grid points in x1, y1, z1

        dim3 grid(n * n, nr, nl);
        evalIntegral_All<<<grid, THREADS_PER_BLOCK>>>(n, nr, hx, hy, hz,
                                                      thrust::raw_pointer_cast(d_out.data()));

        sum = thrust::reduce(d_out.begin(), d_out.end(), 0.0f, thrust::plus<double>());
