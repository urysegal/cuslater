...
__global__ void evaluateIntegrandReduceZ(int nx, int ny, int nz, float r, float l_x, float l_y,
    ...
    if (idx < nx * ny) {
        ...
        float xdiffc_1 = abs(xvalue - d_c[0]); // x1.x - c1.x
        float ydiffc_1 = abs(yvalue - d_c[1]); // x1.y - c1.y
        // h1 ≈ sqrt( (x1.x - c1.x)^2 + (x1.y - c1.y)^2 )
        float h1 = manhatdist(xdiffc_1, ydiffc_1);

        float xdiffc_2 = abs(xvalue - d_c[3]); // x1.x - c2.x
        float ydiffc_2 = abs(yvalue - d_c[4]); // x1.y - c2.y
        // h2 ≈ sqrt( (x2.x - c2.x)^2 + (x2.y - c2.y)^2 )
        float h2 = manhatdist(xdiffc_2, ydiffc_2);

        float xdiffc_3 = abs(xvalue - d_c[6] + r * l_x); // x1.x - c3.x + lx
        float ydiffc_3 = abs(yvalue - d_c[7] + r * l_y); // x1.y - c3.y + ly
        // h3 ≈ sqrt( (x1.x - c3.x + lx)^2 + (x1.y - c3.y + ly)^2 )
        float h3 = manhatdist(xdiffc_3, ydiffc_3);

        float xdiffc_4 = abs(xvalue - d_c[9] + r * l_x);  // x1.x - c4.x + lx
        float ydiffc_4 = abs(yvalue - d_c[10] + r * l_y); // x1.y - c4.y + ly
