...
namespace cuslater {
    ...
    __launch_bounds__(THREADS_PER_BLOCK, 3) __global__
        ...
        if (__builtin_expect(tid < totalXY, 1)) [[likely]] {
            ...
            {
                const int idx0 =  base;
                real_t    z0   = __ldg(&d_z_grid[0]);
                real_t    a12  = __ldg(&alpha12[idx0]);
                real_t    t3   = alpha.x * norm(xdiffc_3, ydiffc_3, z0 - c3.z + rlz);
                real_t    t4   = alpha.y * norm(xdiffc_4, ydiffc_4, z0 - c4.z + rlz);
                v0 += real_t(0.5) * exp_fn(r - (a12 + t3 + t4));
            }
            // k = n-1
            {
                const int idxN1 = (n - 1) * pitchXY + base;
                real_t    zn1   = __ldg(&d_z_grid[n - 1]);
                real_t    a12   = __ldg(&alpha12[idxN1]);
                real_t    t3    = alpha.x * norm(xdiffc_3, ydiffc_3, zn1 - c3.z + rlz);
                real_t    t4    = alpha.y * norm(xdiffc_4, ydiffc_4, zn1 - c4.z + rlz);
                v1 += real_t(0.5) * exp_fn(r - (a12 + t3 + t4));
            }

            constexpr int UNROLL                = 8; // maybe 4 for double?
            int           k                     = 0;
            int           nz                    = n - 2;
            const real_t* __restrict__ zptr     = d_z_grid + 1;             // start at k=1
            const real_t* __restrict__ a12_base = alpha12 + base + pitchXY; // idx = k*pitchXY

            // prefetch first UNROLL alpha12
            real_t a[UNROLL];
#pragma unroll
            for (int t = 0; t < UNROLL && t < nz; ++t) {
                a[t] = __ldg(&a12_base[(k + t) * pitchXY]);
            }

            for (; k + UNROLL - 1 < nz; k += UNROLL) {
                // compute on the prefetched
                real_t z0 = zptr[k + 0], z1 = zptr[k + 1], z2 = zptr[k + 2],
                       z3 = zptr[k + 3];
                real_t z4 = zptr[k + 4], z5 = zptr[k + 5], z6 = zptr[k + 6],
                       z7 = zptr[k + 7];
...
#pragma unroll
                    for (int t = 0; t < UNROLL; ++t) {
                        a[t] = __ldg(&a12_base[(k + UNROLL + t) * pitchXY]);
                    }
                }
            }
            ...
            for (; k < nz; ++k) {
                real_t z   = zptr[k];
                real_t a12 = __ldg(&a12_base[k * pitchXY]);
