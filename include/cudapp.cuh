    ...
    inline std::error_category const& cudaErrorCategory() noexcept {
        static struct : std::error_category {
            ...
            std::string message(int ev) const override {
                const char* p = cudaGetErrorString(static_cast<cudaError_t>(ev));
                std::string msg(p);
                return std::string(cudaGetErrorString(static_cast<cudaError_t>(ev)));
            }
        } category;
...
#define CHECK_CUDA(expr)                                       \
    ...
    struct CudaAsyncPoolDeviceArena {
      ...
      public:
        ...
        CudaAsyncPoolDeviceArena(cudaMemPool_t pool, cudaStream_t stream = nullptr) noexcept :
    ...
    struct CudaStream : GenericResourcePtr<cudaStream_t> {
      ...
      public:
        ...
        void waitEvent(CudaEvent const& event, unsigned int flags = cudaEventWaitDefault) const {
        ...
        void joinAsync(Func&& func) const {
            ...
            cudaStreamCallback_t callback = [](cudaStream_t stream, cudaError_t status, void* userData) {
    ...
    struct CudaAllocator : private Arena {
        ...
        T* allocate(size_t size) {
            void* ptr = nullptr;
            if (sizeof(T) > 1 && size > std::numeric_limits<size_t>::max() / sizeof(T)) [[unlikely]] {
        ...
        template<class... Args>
        static std::enable_if_t<sizeof...(Args)> construct(T* p, Args&&... args) noexcept(
            noexcept(std::construct_at(p, std::forward<Args>(args)...))) {
            std::construct_at(p, std::forward<Args>(args)...);
        }

        static void construct(T* p) noexcept(noexcept(std::construct_at(p))) {
        ...
        construct(T* p, Args&&... args) noexcept(noexcept(::new(static_cast<void*>(p))
                                                              T(std::forward<Args>(args)...))) {
            ::new (static_cast<void*>(p)) T(std::forward<Args>(args)...);
        }

        static void construct(T* p) noexcept(noexcept(::new(static_cast<void*>(p)) T)) {
        ...
        template<class U>
        constexpr CudaAllocator(CudaAllocator<U, Arena> const& other) noexcept {}

        template<class U>
        constexpr bool operator==(CudaAllocator<U, Arena> const& other) const noexcept {
            return true;
        }
    ...
    struct CudaVectorResizer {
      ...
      public:
        explicit CudaVectorResizer(Vector& vec) noexcept : m_vec(vec), m_size(vec.size()) {}
    ...
    __global__ void paralelFor(Index n, Func func) {
        for (Index i = blockIdx.x * Index(blockDim.x) + threadIdx.x; i < n;
